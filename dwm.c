...
static const char dwmdir[] = "dwm-sky";
...
/* Default (fallback) config paths: ~/.local/share/dwm-sky/config/ */
		...
		if (pid == 0) {
			...
			if (home) {
				snprintf(script, sizeof(script),
				         "%s/.local/share/dwm-sky/scripts/theme-apply.sh", home);
...
setup_inotify(void)
{
	...
	/* User-editable config: ~/.config/dwm-sky/ */
	snprintf(toml_config_dir,   sizeof(toml_config_dir),
	         "%s/.config/dwm-sky", home);
	...
	/* Default (fallback) config: ~/.local/share/dwm-sky/config/ */
	snprintf(toml_default_dir,             sizeof(toml_default_dir),
	         "%s/.local/share/dwm-sky/config", home);
