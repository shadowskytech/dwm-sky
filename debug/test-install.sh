#!/bin/bash
# test-install.sh — Integration test for dwm-sky TOML config loading
#
...
seed_defaults() {
    local fake_home="$1"
    local def_dir="$fake_home/.local/share/dwm-sky/config"
    mkdir -p "$def_dir"
...
seed_user() {
    local fake_home="$1"
    local usr_dir="$fake_home/.config/dwm-sky"
    mkdir -p "$usr_dir"
...
echo ""
echo "╔════════════════════════════════════════════╗"
echo "║   dwm-sky TOML config integration test  ║"
echo "╚════════════════════════════════════════════╝"
echo ""
...
printf '[[invalid\nthis is not valid toml ===\n' \
    > "$FAKE3/.config/dwm-sky/themes.toml"
printf '[[invalid\n' \
    > "$FAKE3/.config/dwm-sky/hotkeys.toml"
