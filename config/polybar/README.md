# Polybar Configuration (dwm-sky)

This directory contains the Polybar status bar configuration bundled with dwm-sky. It is **self-contained** — you do not need to clone any external repos.
...
When you run `sudo make install` from the dwm-sky root, Polybar configs are copied to:
```
~/.config/polybar/
```
